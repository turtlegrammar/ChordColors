import { Options, defaultOptions, CanvasOptions, OvertoneConfig, WheelViewModeConfig, CircleConfig, MixBias, RenderBias } from '../domain/models/options';
...
export type RandomPixelatorConfig = {circleConfig: CircleConfig; mixConfig: MixBias; renderConfig: RenderBias; overtoneConfig: OvertoneConfig; }
...
export function makeRandomPixelatorConfig(config: Options) {
    return { circleConfig: config.circle, mixConfig: config.mix, renderConfig: config.render, overtoneConfig: config.overtone}
};
...
function generateRGBs(midiBatcher: MidiBatcher, config: RandomPixelatorConfig) : RGB[] {
